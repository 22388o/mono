...
with lib; let
  options = {
    services.bitcoind = {
      ...
      onionPort = mkOption {
        ...
        default =
          if (config.nix-bitcoin.onionServices.bitcoind.enable or false)
          then 8334
          else null;
        description = mdDoc ''
      ...
      proxy = mkOption {
        ...
        default =
          if cfg.tor.proxy
          then config.nix-bitcoin.torClientAddressWithPort
          else null;
      ...
      zmqpubhashblock = mkOption {
        ...
        default = null;
        example = "tcp://127.0.0.1:28332";
        description = mdDoc "ZMQ address for zmqpubhashblock notifications";
      };
      zmqpubhashtx = mkOption {
        type = types.nullOr types.str;
        default = null;
        example = "tcp://127.0.0.1:28332";
        description = mdDoc "ZMQ address for zmqpubhashtx notifications";
      };
      ...
      zmqpubsequence = mkOption {
        type = types.nullOr types.str;
        default = null;
        example = "tcp://127.0.0.1:28333";
        description = mdDoc "ZMQ address for zmqpubsequence notifications";
      };
  ...
  configFile = builtins.toFile "bitcoin.conf" ''
    ...
    ${optionalString (cfg.sysperms != null) "sysperms=${
      if cfg.sysperms
      then "1"
      else "0"
    }"}
    ${optionalString (cfg.disablewallet != null) "disablewallet=${
    ...
    # Connection options
    listen=${
      if (cfg.listen || cfg.listenWhitelisted)
      then "1"
      else "0"
    }
    ...
    ${optionalString (cfg.discover != null) "discover=${
      if cfg.discover
      then "1"
      else "0"
    }"}
    ...
    ${optionalString (cfg.zmqpubhashtx != null) "zmqpubhashtx=${cfg.zmqpubhashtx}"}
    ...
    ${optionalString (cfg.zmqpubsequence != null) "zmqpubsequence=${cfg.zmqpubsequence}"}
...
in {
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.bitcoind = {
      ...
      preStart = let
        extraRpcauth = concatMapStrings (
          name: let
            user = cfg.rpc.users.${name};
          in
            optionalString user.passwordHMACFromFile ''
