{ nodejs_20, jq, openssl, coreutils, stdenv, mkShell, lib, fetchurl, writeText, writeShellScript, runCommand, fetchFromGitHub }:
rec {
  default_nodejs = nodejs_20;
    ...
    let
      ...
      patchedResolved =
        if (spec.link or false) then
          {
            resolved = "file://${sourceOptions.srcDir + sourceOptions.localModulesMappings.${raw_name}}"; # TODO: throw if not found
            # resolved = "file://${sourceOptions.localModulesMappings.${raw_name}}"; # TODO: throw if not found
            integrity = null;
            link = true;
          }
    ...
    , ...
      ...
      let
        ...
        sourceOptions = {
          ...
          inherit nodejs sourceOverrides srcDir localModulesMappings;
      ...
      {
        inherit packagefile sourceOptions allDependenciesNames patchedLockfilePath patchedPackagefilePath;
      };
  ...
  node_modules =
    ...
    , ...
      ...
      assert (preInstallLinks != null) ->
        throw "`preInstallLinks` was removed use `sourceOverrides";
      let
        cleanArgs = builtins.removeAttrs args [ "src" "packageJson" "packageLockJson" "buildInputs" "nativeBuildInputs" "nodejs" "preBuild" "postBuild" "sourceOverrides" "githubSourceHashMap" "localModulesMappings"];
        lockfile = readPackageLikeFile packageLockJson;
        packagefile = readPackageLikeFile packageJson;
      ...
      stdenv.mkDerivation ({
        ...
        buildPhase = ''
          runHook preBuild
          export HOME=.
          npm ci --nodedir=${nodeSource nodejs} --ignore-scripts --loglevel verbose
