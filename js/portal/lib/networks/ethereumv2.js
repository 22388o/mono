 ...
 * @type {Ethereum}
 */
module.exports = class EthereumV2 extends Network {
  constructor(props) {
  ...
  async open(party, opts) {
    if (party.isSecretSeeker) {
      ...
      const invoice = await this._doDeposit(
        `0x${party.swap.secretHash}`,
        {
          recipient: party.swap.recipient,
          tokenDeposit: party.swap.depositToken,
          tokenDesire: party.swap.desireToken,
          amountDeposit: party.swap.depositAmount,
          amountDesire: party.swap.desireAmount,
          networkDeposit: party.swap.depositNetwork,
          networkDesire: party.swap.desireNetwork,
        },
        opts.ethereum
      );
      ...
      const subscription = this.eventDeposit.watch((err, deposit) => {
        ...
        debug(
          party.id,
          "(secretHolder) got deposit event, and has stopped watching"
        );

        if (err != null) {
          debug(
            party.id,
            "(secretHolder) got an error waiting for deposit",
            err
          );
          ...
          debug(
            party.id,
            `(secretHolder) is settling the ${this.name} invoice using secret "${opts.secret}"`
          );
          ...
          this._settleInvoice(`0x${opts.secret}`, opts.ethereum)
            .then((...args) =>
              debug(
                party.id,
                `(secretHolder) has settled the ${this.name} invoice`,
                ...args
              )
            )
    ...
    return new Promise((resolve, reject) => {
      if (party.isSecretSeeker) {
        const subscription = this.eventClaim.watch((err, claim) => {
          ...
          debug(
            party.id,
            "(secretSeeker) got claim event, and has stopped watching"
          );

          if (err != null) {
            debug(
              party.id,
              "(secretSeeker) got an error waiting for claim",
              err
            );
            reject(err);
          } else {
        ...
        debug(
          party.id,
          `(secretSeeker) is paying the ${this.name} invoice bearing id ${invoiceId} with secret hash`,
          secretHashHex
        );
        this._payInvoice(secretHashHex, opts.ethereum, party.quantity)
          .then((receipt) => {
            debug(
              party.id,
              `(secretSeeker) has paid the ${this.name} invoice`,
              receipt
            );
        ...
        debug(
          party.id,
          "(secretSeeker) is now waiting for funds to be claimed"
        );
      } else if (party.isSecretHolder) {
  ...
  _doDeposit(secretHash, depositReq, keys) {
    const method = "deposit";
    const params = [secretHash, depositReq];
    return this._callSolidity(method, params, keys);
  }

  _payInvoice(secretHash, keys, amount) {
    const method = "payInvoice";
    const params = [secretHash];
    return this._callSolidity(method, params, keys, `0x${amount.toString(16)}`);
  }

  _settleInvoice(secret, isDepositor, keys) {
    const method = "claim";
    const params = [secret, isDepositor];
    return this._callSolidity(method, params, keys);
  }
