...
const Web3legacy = require("web3melnx");
...
class ChannelClient extends EventEmitter {
  constructor(props) {
    ...
    const web3new = new Web3(); // new Web3.providers.HttpProvider(props.url)
    this.web3new = web3new;

    const web3 = new Web3legacy(
      new Web3legacy.providers.HttpProvider(props.rpc)
    );
    web3.chainId = props.chainId;
    this.web3 = web3;

    this.channelContractAddress = "0x8a14863aDEE8926edD56f263d026AD2816f1C983";
    const abi = require("./abi_multichannel.js");
    this.channelContract = this.web3.eth
      .contract(abi)
      .at(this.channelContractAddress);
  ...
  connect() {
    ...
    this.subscriber = new cote.Subscriber({
      name: "client subscriber " + this.name,
    });
    this.publisher = new cote.Publisher({
      name: "client publisher " + this.name,
    });

    this.subscriber.on("transfersigs", (update) =>
      this.handleSignaturesRevealed(update)
    );
  ...
  async loadChannel(sourceAddress, targetAddress, tokenAddress = "0x00") {
    ...
    const chan = await this.channelContract.channels(channelId);

    console.log(this.name, "CHANNEL FROM CHAIN", chan);

    const sender = chan[0];
    const receiver = chan[1];
    const deposited = chan[2];
    const claimed = chan[3];
    if (
      !(
        sender.toLowerCase() == sourceAddress.toLowerCase() &&
        receiver.toLowerCase() == targetAddress.toLowerCase()
      )
    ) {
  ...
  async depositToChannel(channelId, amount) {
    return await this._callSolidity(
      "deposit",
      [channelId],
      this.keypair,
      amount
  ...
  getChannelBalance(channel) {
    ...
    if (channel.spender == this.address)
      return this.getChannelCapacity(channel);
    return null;
  }
  ...
  concatHex() {
    ...
    return (
      "0x" + args.map((x) => (x.startsWith("0x") ? x.substr(2) : x)).join("")
    );
  }

  async sendTransfer(transfer) {
    let channel = await this.getOrLoadChannel(
      this.address,
      transfer.targetAddress,
      transfer.tokenAddress
    );
    console.log(
      `transfer.targetAddress: ${transfer.targetAddress}; transfer.tokenAddress: ${transfer.tokenAddress}`
    );
    if (!channel) {
    ...
    if (!transfer.hashOfSecret) {
      //if a hash of secret is not provided, generate one and store it
    ...
    console.log(
      this.name,
      "NEW CHANNEL BALANCE PENDING",
      this.getChannelBalance(channel)
    );

    this.publisher.publish("transfer", transfer);
    this.publisher.publish("signed", {
      hashOfSecret: transfer.hashOfSecret,
      signature: this.signMessage(this.encodeTransferForSig(transfer))
        .signature,
  ...
  getChannel(sourceAddress, targetAddress, tokenAddress = "0x00") {
    return this.channels[
      this.getChannelId(sourceAddress, targetAddress, tokenAddress)
    ];
  }
  ...
  registerSecret(secret) {
    const hash = this.computeHash(secret);
    this.secrets[hash] = secret;
  }
  ...
  payInvoice(invoice) {
    if (typeof invoice === "string") invoice = this.invoices[invoice.id];
      ...
      const lookedup = this.invoices[invoice.id];
  ...
  async sendTransfer(transfer) {
    const channel = await this.getOrLoadChannel(
      this.address,
      transfer.targetAddress,
      transfer.tokenAddress
    );
    ...
    console.log("CHANNEL", channel);
    const capacity = this.getChannelCapacity(channel);
  ...
  handleSecretRevealed(update) {
    ...
    const hashOfSecret = this.computeHash(secret);
  ...
  async handleSignaturesRevealed(revealed) {
    ...
    const hash = revealed.hashOfSecret;
    const received = this.received[hash];

    if (!received) {
      console.log(
        this.name,
        "IGNORING SIGS, HAVENT RECEIVED PAYMENT FOR THIS HASH",
        revealed.hashOfSecret
      );
    ...
    const message = this.encodeTransferForSig(received);
    const recovered = this.verifyMessage(
      revealed.signatures[received.sourceAddress],
      message,
    ...
    if (recovered.toLowerCase() != received.sourceAddress.toLowerCase()) {
      ...
      this.signatures[revealed.hashOfSecret] = revealed.signatures;
      this.knownPaymentSignatures[revealed.hashOfSecret] =
        revealed.signatures[received.sourceAddress];
  ...
  handleTransfer(update) {
    if (update.invoiceId && update.sourceAddress == this.address)
      this.emit("invoicePaidByMe", update);
    ...
    if (!this.secrets[update.hashOfSecret]) {
      // if secret is not known just send back same amount of token
      ...
      // if you know the secret and received counterpayment, reveal secret to network
  ...
  handleInvoice(invoice) {
    ...
    if (!invoice.hashOfSecret) {
      // TODO: default behavior for original invoice
    } else {
      // TODO: default behavior for original invoice
    }
    ...
    if (this.testMode) {
      ...
      console.log(
        this.name,
        "RECEIVED INVOICE, AUTOMATICALLY PAYING IT",
        invoice
      );

      const transfer = this.payInvoice(invoice);
  ...
  decToUint256Hex(value) {
    const res = parseInt(value).toString(16).padStart(64, "0");
    return res;
  }

  encodeTransferForSig(params) {
    const message = this.concatHex(
      this.channelContractAddress,
      params.channelId,
      this.decToUint256Hex(params.spent)
    );
  ...
  signMessage(msg) {
    ...
    try {
      ...
      // console.log('msg : ', msg);
      const sign = this.web3new.eth.accounts.sign(msg, this.keypair.private);
  ...
  verifyMessage(signature, message, sender) {
    ...
    try {
      ...
      if (recoveredAddr.toLowerCase() === from.toLowerCase()) {
        console.log(
          this.name,
          `Successfully ecRecovered signer as ${recoveredAddr}`
        );
      } else {
        console.log(
          this.name,
  ...
  async _callSolidity(funcName, funcParams, keys, ethValue) {
    return new Promise((resolve, reject) => {
      ...
      const contract = this.channelContract;
  ...
  createCounterInvoice(update, hashOfSecret) {
    if (update.originalInvoiceId) {
      console.log(
        this.name,
        "INVOICE ALREADY A COUNTER INVOICE, NOT COUNTER-INVOICING"
      );
      return;
    }
    ...
    this.createInvoice({
      ...
      hashOfSecret: update.hashOfSecret || hashOfSecret,
      payee: this.address,
  ...
  sendCounterPayment(update) {
    ...
    this.sendTransfer({
      tokenAddress: update.tokenAddress,
      amount: update.amount,
      hashOfSecret: update.hashOfSecret,
      targetAddress: update.sourceAddress,
    });
  }
}
