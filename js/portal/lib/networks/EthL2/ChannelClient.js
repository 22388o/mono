...
class ChannelClient extends EventEmitter {
  ...
  async loadChannel (sourceAddress, targetAddress, tokenAddress = '0x00') {
    ...
    const sender = chan[0]; const receiver = chan[1]; const deposited = chan[2]; const claimed = chan[3]
  ...
  getChannelId (sourceAddress, targetAddress, tokenAddress = '0x00') {
    const buffer = this.concatHex(this.channelContractAddress, sourceAddress, targetAddress)
  ...
  async sendTransfer (transfer) {
    const channel = await this.getOrLoadChannel(this.address, transfer.targetAddress, transfer.tokenAddress)
    if (!channel) {
  ...
  async handleSignaturesRevealed (revealed) {
    ...
    const recovered = this.verifyMessage(revealed.signatures[received.sourceAddress], message, received.sourceAddress)
