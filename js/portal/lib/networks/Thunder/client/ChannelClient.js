...
class ChannelClient extends EventEmitter {
  constructor (props) {
    ...
    const web3 = new Web3legacy(new Web3legacy.providers.HttpProvider(props.rpc))
    ...
    const channelAbi = require('./abi/abi_multichannel_erc20.js')
    this.channelContract = this.web3.eth.contract(channelAbi).at(this.channelContractAddress)

    const l1SwapsAbi = require('./abi/abi_L1swap')
  ...
  async loadChannel (sourceAddress, targetAddress, tokenAddress) {
    const channelId = this.getChannelId(sourceAddress, targetAddress, tokenAddress)
  ...
  async _loadChannel (channelId) {
    ...
    const sender = chan[0]; const receiver = chan[1]; const deposited = chan[2]; const claimed = chan[3]; const token = chan[4]
  ...
  async depositToChannel (channelId, amount, tokenAddress) {
    ...
    const value = tokenAddress == NULL_ADDRESS ? amount : '0x0'
  ...
  async openChannel (targetAddress, capacity, tokenAddress) {
    ...
    const value = tokenAddress == NULL_ADDRESS ? capacity : '0x0'
  ...
  async depositL1Payment (receiver, hashOfSecret, amount, tokenAddress) {
    if (tokenAddress == NULL_ADDRESS) {
      ...
      const result = await this._callSolidity('depositEth', [
        tokenAddress,
        amount,
        5,
        hashOfSecret,
  ...
  getChannelId (sourceAddress, targetAddress, tokenAddress) {
    ...
    const buffer = this.concatHex(this.channelContractAddress, sourceAddress, targetAddress, tokenAddress)
  ...
  registerSecret (secret) {
    const hash = this.computeHashOfSecret(secret)
  ...
  async sendTransfer (transfer) {
    ...
    const nextAddress = transfer.path
      ? transfer.path[transfer.path.indexOf(this.address) + 1]
      : transfer.targetAddress

    console.log(this.name, 'NEXT ADDRESS', nextAddress)

    const channel = await this.getOrLoadChannel(this.address, nextAddress, transfer.tokenAddress)
    if (!channel) {
    ...
    const chan = this.updateChannelState(channel, { transfer }, true)
    ...
    const encodedTx = this.encodeTransferForSig(transfer)
    const sig = transfer.signature || this.signMessage(encodedTx).signature
  ...
  handleSecretRevealed (update) {
    ...
    const hashOfSecret = this.computeHashOfSecret(secret)
  ...
  async handleSignaturesRevealed (revealed) {
    ...
    const recovered = this.ecRecover(revealed.signatures[received.sourceAddress], message, received.sourceAddress)
    ...
    if (recovered.toLowerCase() != received.sourceAddress.toLowerCase()) {
      ...
      const channel = await this.getOrLoadChannel(received.sourceAddress, received.targetAddress, received.tokenAddress)
  ...
  async handleTransfer (transfer) {
    ...
    if (transfer.path && this.address != transfer.path[transfer.path.length - 1]) {
      ...
      const claimedEvent = this.l1SwapsContract.Claimed({ secretHash: transfer.hashOfSecret }, { fromBlock: this.web3.blockNumber })
      claimedEvent.watch((error, result) => {
        ...
        const secretHex = ethers.toBeHex(result.args.secret.toFixed())
        const secretHashHex = ethers.toBeHex(result.args.trade.toFixed())
  ...
  encodeTransferForSig (params) {
    const message = this.concatHex(this.channelContractAddress, params.channelId, this.decToUint256Hex(params.spent))
    // console.log("ENCODED TX", message);
    const hash = this.web3new.utils.keccak256(message)
  ...
  async _callSolidity (funcName, funcParams, keys, ethValue, contr) {
    return new Promise((resolve, reject) => {
      ...
      const contract = contr || this.channelContract
  ...
  setupRpc (opts) {
    ...
    app.get('/channel_balance', async (req, res) => {
      ...
      const channel = await this.getOrLoadChannelById(id)
    ...
    app.get('/channel/open', async (req, res) => {
      ...
      const result = await this.openChannel(target, capacity, token)
    ...
    app.get('/transfer', async (req, res) => {
      ...
      const submarine = req.query.submarine
      ...
      const transfer = await this.sendTransfer({
        ...
        signature,
        submarineSwap: !!submarine
      })

      if (transfer) {
        const secret = this.secrets[transfer.hashOfSecret]
