...
let client1 = new ChannelClient({ keypair: keypairs[0], rpc: config.web3.goerli.rpc_url, chainId: config.web3.goerli.chain_id });
if(!clientId || clientId=='1') client1.connect();
let client2 = new ChannelClient({ keypair: keypairs[1], rpc: config.web3.goerli.rpc_url, chainId: config.web3.goerli.chain_id });
if(!clientId || clientId=='2') client2.connect();
let client3 = new ChannelClient({ keypair: keypairs[2], rpc: config.web3.goerli.rpc_url, chainId: config.web3.goerli.chain_id });
...
async function spamPayment(){   
    ...
    let transfer = await client1.sendTransfer({
        tokenAddress:ETH, amount:'11',
        targetAddress: client3.address,//client2.address,
        path: client1.getMultihopPath(client1.address, client3.address),
    });

    if(transfer){
        let secret = client1.secrets[transfer.hashOfSecret];
...
async function settleChannel(client){
    client.on('signatures', async tx => {
        ...
        let result = await client3.settleChannel(tx.channelId, tx.spent, tx.signature)
