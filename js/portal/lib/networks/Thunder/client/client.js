...
let keys = null
let rpc_port = null
const keypairIndex = process.env[envVars.keypairIndex]
const keypairFile = process.env[envVars.keypairFile]
...
if (keypairIndex) {
  ...
  const json = fs.readFileSync(keypairFile, 'utf8')
  let keys = JSON.parse(json)
  const l2keys = keys.ethl2
  keys = {
...
const notProvidedPrefix = 'Please provide ENV variables: '
