/**
 * @file Behavioral specification for an Arbitrum/Lightning atomic swap
 */
...
/**
 * This is a simple test case wherein we swap ETH and BTC using arbitrum and
 * lightning.
 */
describe.only('Swaps - Arbitrum/Lightning', function () {
  ...
  it('must broadcast the committed swap to Alice and Bob', function () {
    ...
    expect(swap.secretSeeker.id).to.be.a('string').that.equals(bob.id)
    expect(swap.secretSeeker.state).to.be.an('object')
    expect(swap.secretSeeker.state['arbitrum.eth']).to.be.an('object')
    expect(swap.secretSeeker.state['arbitrum.eth'].invoice).to.be.an('object')
  })
})
