  ...
  it.only('must allow Alice to place an order', async function (done) {
    await this.test.ctx.playnet.apps.alice
      .once('order.created', order => {
        ...
        // // expect(order.statusNumber).to.be.a('string').that.equals('created')
      })
      .once('created', order => {
        console.log("alice created event received", order)
      })
      .once('opened', order => {
        console.log("alice opened event received", order)
      })
