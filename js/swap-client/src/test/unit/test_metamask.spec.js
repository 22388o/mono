...
describe('Metamask Wallet Test', async () => {
  let browser, projPage;
  before('Open Test Browser', async () => {
    const metamaskExtPath = path.join(process.cwd(), 'src/test/crx/metamask');

    browser = await puppeteer.launch({
      headless: 'new',
      //headless: false,
      args: [
        `--disable-extensions-except=${metamaskExtPath}`,
        `--load-extension=${metamaskExtPath}`
      ]
    });
  ...
  describe('Create new metamask wallet', async () => {
    let metamaskPage;

    it('Input passwords', async () => {
      metamaskPage = (await browser.pages())[1];
  ...
  describe('Connect metamask wallet to the project', async () => {
