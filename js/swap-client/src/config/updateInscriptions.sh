...
# Process the data
output=$(jq -c "[.[$start_index:][] | {title: (\"Inscription #\" + (\".\") + tostring), type: \"BTCORD-1\", network: \"btc.btc\", detail: \"Inscription\", rate: 1, isNFT: true, img_url: (\"http://localhost/content/\" + .inscription), balance: 1, options: [{type: \"paddingAmount\", title: \"Padding Amount\", value: 4000}], info: {inscription: .inscription, location: .location, explorer: .explorer}}]" $inputFile | jq -M '.')

# Generate unique title for each item based on its position
output=$(echo "$output" | jq '[foreach .[] as $item (0; .+1; . as $i | $item | .title = "Inscription #\($i+1)")]' )
