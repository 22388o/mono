...
contract PaymentChannel is ReentrancyGuard {
    ...
    function getChannelId(
        ...
        address _receiver,
        address _token
    ) public view returns (uint channelId) {
        return uint(getChannelIdHex(_sender, _receiver, _token));
    }

    // solhint-disable-next-line max-line-length
    function getChannelIdHex(
        address _sender,
        address _receiver,
        address _token
    ) public view returns (bytes32 channelId) {
        return keccak256(getChannelIdPreimage(_sender, _receiver, _token));
    }

    // solhint-disable-next-line max-line-length
    function getChannelIdPreimage(
        address _sender,
        address _receiver,
        address _token
    ) public view returns (bytes memory channelInfo) {
    ...
    function _openChannel(
        ...
        uint _amount,
        address _token
    ) internal returns (uint _channelId) {
    ...
    function openChannel(
        ...
        uint _amount,
        address _token
    ) external payable returns (uint channelId) {
    ...
    function _verify(
        ...
        return
            _getEthSignedHash(_channelId, _amount).recover(_sig) ==
            channels[_channelId].sender;
    }
